...
int n,a[maxn],ans; set<int> card; bool cnt[maxn*2];
...
void solve(){
    ...
    for (int i=1;i<=2*n;++i){
        if (cnt[i]==false) card.ins(i);
    }
    for (int i=0;i<n;++i){
        auto it = card.ub(a[i]);
        if (it==card.end()) it=card.begin();
        ans+= *it > a[i];
        card.erase(it);
    }
    cout << ans << endl;
...
int main(){
	...
	*/
	solve();
	cerr<< setprecision(5) << "time taken : "<<(float)clock()/CLOCKS_PER_SEC <<" secs"<<endl;
	return 0;
}
